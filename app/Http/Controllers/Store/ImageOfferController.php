...
class ImageOfferController extends Controller
{
...

    {
        ...
        $offer_images = ImageOffer::where('offer_id', $id)->paginate(8);
        return view('store.image_offer.index', compact('offer_images', 'offer'));
    }
    ...
    {
        $offer = Offer::findOrFail($id);
        return view('store.image_offer.create', compact('offer'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('images')) {
            foreach ($request->file('images') as $image) {
                $img = $image->store('image_offer', 'public');
                ImageOffer::create([
                    'image' => 'storage/' . $img,
    ...
    public function edit(ImageOffer $imageOffer, $id)
    {
        ...
        return view('store.image_offer.edit', compact('ImageOffer', 'Offer'));
    }
