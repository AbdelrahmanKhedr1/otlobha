                            ...
                            <div class="chat">
                                <div class="chat-container">
                                    <div class="user-bar">
                                        ...
                                        <div class="name">
                                            {{-- <span>{{ $messages->conversation->store->name ??  $messages->conversation->customer->name }}</span> --}}
                                        </div>
                                    ...
                                    <div class="conversation">
                                        <div class="conversation-container" id="conversationContainer">
                                            ...
                                            @foreach ($messages as $message)
                                                <div
                                                    class="message {{ $message->is_admin == false ? 'received' : 'sent' }}">
                                                    {{ $message->text }}
                                                    <span class="metadata">
                                                        <span
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            // دالة لتحريك الـ scroll إلى أسفل
            ...
            window.Echo.private(`chat.store.${conversationId}`)
                .listen('.chatEvent', (data) => {
                    const messageClass = data.message.is_admin == false ? 'received' : 'sent';
                    const messageHtml = `
            ...
            $('.conversation-compose').on('submit', function(e) {
                ...
                $.ajax({
                    url: "{{ route('conversation.sendMessage',$conversation->id) }}",
                    method: 'POST',
                    headers: {
            ...
            // تحريك الـ scroll لأسفل عند تحميل الصفحة
            scrollToBottom();
